...
import { getStoreHours, addStoreHour, updateStoreHour, deleteStoreHour } from "@/api/AdminServices";
...
const StoreHoursTable = ({ reload, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        const hourGroup = groupedData.find(group => group.ids.includes(currentId));
        if (hourGroup) {
      ...
      Swal.fire('Error', error.response?.data?.message || 'Failed to save store hours', 'error');
    }
  };
  ...
  const handleDelete = async (ids) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire('Error', error.response?.data?.message || 'Failed to delete store hours', 'error');
      }
    }
  };
                  ...
                  <tbody>
                    {currentItems.length > 0 ? (
                      currentItems.map((item, index) => (
                        <tr key={`${item.name}-${item.opening_time}-${item.closing_time}`}>
                      ...
                      <tr>
                        <td colSpan="6" className="text-center py-4">
                          {searchTerm ? 'No matching store hours found' : 'No store hours available'}
                        </td>
                      </tr>
                    )}
                  </tbody>
      ...
      <div className="modal-content">
        <form onSubmit={handleSubmit}>
          <div className="modal-header">
            <h5 className="modal-title">{isEditMode ? 'Edit Store Hours' : 'Add New Store Hours'}</h5>
            <button
              type="button"
              className="btn-close"
              onClick={closeModal}
