...
const ToppingTable = ({ reload, onSuccess }) => {
  ...
  // Function to format price with current currency
  const formatPrice = async (priceValue) => {
    try {
      // Assuming base currency is USD, adjust if different
  ...
  // Component to handle async price formatting
  ...
  const handleToggleDeleteRestore = async (item) => {
    ...
    try {
      if (isCurrentlyActive) {
        ...
        await reactivateTopping(item.id);
        onSuccess && onSuccess("Topping restored!");
      }
